...
def main():
	...
	ssdata = getSSID(debug,conss,cursorss,ssdate,sstype)
	ssid = ssdata[0]
	...
	hwconfigs = getHWConfigurations(debug,baseurl)  #returns list of tuple = (deviceid,bucketid,graphicsbrand,finisheddate,lastupdatedon)
	loadDeviceIds(debug,conss,cursorss,ssid,hwconfigs)
	...
	for hwconfig in hwconfigs_unique: 
		...
		print '\n'
		games = getGamesByDeviceId(debug,deviceid,baseurl)   # returns tuple = (taskid,salesforceid,name,lastupdatedon,finisheddate,releasedate)
		for game in games: 
			taskid = game[0]
			salesforceid = game[1]
			name = game[2]
			...
			sstitleid = getTitleId(debug,conss,cursorss,ssid,taskid,name,finisheddate,releasedate,salesforceid,lastupdatedon)
			writethumbnail(debug,conss,cursorss,ssid,tmppath,ssrepopath,thumbnail,taskid,sstitleid,ssarchident,conbe,cursorbe)
			getzipscreenshots(debug,tempzipdir,taskid,deviceid,baseurl)
			ssbmid = getBenchmarkId(debug,conss,cursorss,sstitleid,bucketid,ssid)
			num_screenshots = processscreenshots(debug,conss,cursorss,ssbmid,sstitleid,tempzipdir,taskid,bucketid,sstype,ssdate,ssidentifier,ssrepopath,ssarchident,ssid,conbe,cursorbe)
			print 'num_screenshots:',num_screenshots
			print '\n'
		print '\n\n'
	print '\n'
	print 'Processing configurations files'
	configs = getGamesListWithConfig(debug,baseurl)
	for config in configs:
		print '\n'
		taskid = config
		print '----------------------------------------------------------------------------'
		print 'taskid:',taskid
		getConfig(debug,taskid,ssarchident,tempzipdir,baseurl)
		processconfigs(debug,conss,cursorss,ssid,tempzipdir,ssrepopath,ssarchident,taskid)
	comparedbs(debug,conss,cursorss,ssid)

...
def data_rows_count():# to count the rows in the database
    con = connect()
    cur=con.cursor()
    cur.execute("SELECT * FROM tbltitles")
    rows=cur.fetchall()
    i=0
    for r in rows:
        i+=1
...
class MyFrame(wx.Frame):# this is the parent frame
    def __init__(self, *args, **kwds):
        ...
        actMenu = wx.Menu()
        actMenu.Append(11, _("Refresh IGGEQAdb"), "", wx.ITEM_NORMAL)
	actMenu.AppendSeparator()
	actMenu.Append(12, _("Snapshot Dev"), "", wx.ITEM_NORMAL)
	actMenu.Append(12, _("Snapshot TST"), "", wx.ITEM_NORMAL)
	actMenu.Append(12, _("Snapshot PRD"), "", wx.ITEM_NORMAL)
        self.frame_1_menubar.Append(actMenu,_("Activities"))
        rptMenu = wx.Menu()
        rptMenu.Append(21, _("Readiness"), "", wx.ITEM_NORMAL)
        self.frame_1_menubar.Append(rptMenu,_("Reports"))
        self.SetMenuBar(self.frame_1_menubar)
        self.__set_properties()
        self.__do_layout()
        self.Bind(wx.EVT_MENU, self.open_dialog, id=11)
        self.Bind(wx.EVT_MENU,self.open_dialog1,id =21)

    def __set_properties(self):
        self.SetTitle(_("IGGE QA Tools"))
